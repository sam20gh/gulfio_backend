        ...
        for (let i = 0; i < activeUsers.length; i += batchSize) {
            ...
            await Promise.all(batch.map(async (userId) => {
                try {
                    ...
                    // Atlas Vector Search — mirrors routes/videos.js. The reels
                    // vectorSearch index is named 'default' and has no filter fields,
                    // so exclusions run as a post-search $match.
